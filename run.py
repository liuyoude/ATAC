...
from dataset import ASDDataset, augment_dict
import utils
...
def main(args):
    ...
    result_path = os.path.join('./results', f'{args.version}',
                               'result_dirichlet.csv' if args.dirichlet_score else 'result.csv')
    reverse = False
    for (train_dir, add_dir, valid_dir, test_dir) in zip(
        ...
        args.num_classes = len(augment_dict[args.machine])
        args.w_normal = 1  # args.num_classes - 1
        args.logger.info(f'NumClasses: {args.num_classes}')
        args.dirichlet_path = os.path.join(args.writer.log_dir, 'dirichlet', f'{args.machine}_dirichlet_alphas.db')
        # train_dataset = ASDDataset([train_dir], args)
        train_dataset = ASDDataset([train_dir, add_dir], args)
        train_dataloader = DataLoader(train_dataset, batch_size=args.batch_size,
                                      shuffle=True, num_workers=args.num_workers)
        # set model
        net = STgramMFN(num_classes=args.num_classes, n_mels=args.n_mels, sec=args.win_secs, arcface=args.arcface,
                        m=args.m[args.machine], s=args.s, sub=args.sub)
        if args.dp:
        ...
        # dirichlet
        if args.dirichlet_score and not os.path.exists(args.dirichlet_path):
            trainer.cal_dirichlet(train_dir)
        # valid model
